...
2026-05-28 level 1 reward transition fix: fixed the X God sword reward path so equipping the sword during the boss reward sequence does not immediately sync React state and remount Phaser while still on Level 1. The reward sequence now defers the power sync until the same state update that advances to Level 2. `npm run build` passes. Browser verification in `output/codex-level1-reward-transition-fixed/` confirms the game is in Level 2 / `MAUSOLEUM ROW` by 13s and 17s, `rewardUnlocked: true`, sword `equipped: true`, and `failedAssets: []`.

2026-05-28 gameplay run-through: rebuilt and ran browser smoke coverage for the start screen, mobile landscape, the Level 1 reward-to-Level-2 transition, direct Level 1-4 combat boots, all five playable characters, and forced boss arenas for Levels 1-4. No console errors or failed assets were found in the sampled runs. Found and fixed one mobile UX issue where phone portrait showed the tiny title screen instead of the landscape prompt; the existing `Turn Sideways` overlay now applies to start and character select as well as gameplay. Latest focused artifacts live in `output/codex-gameplay-runthrough-final/`.

2026-05-28 iPad/mobile control fix: split the production play UI from the legacy desktop testing UI. Customer-facing gameplay now shows compact HUD plus touch controls at tablet/iPad Pro sizes instead of hiding controls behind the `lg` breakpoint. The legacy desktop layout with side panels, workshop, and keyboard-only play is still available with `?desktopControls=1` or `?legacyDesktop=1`. `npm run build` passes. Browser verification in `output/codex-ipad-customer-vs-legacy/` confirms iPad Pro customer mode has 7 touch controls and moves X God with a held right button, while legacy mode has 0 touch controls and shows the desktop panels.
